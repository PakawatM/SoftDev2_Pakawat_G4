...
def students_planning_to_register(request):
    # Search subject
    if 'search-subject' in request.GET:
        q = request.GET.get('search-subject')
        #subject = Subject.objects.filter(subject_ID__icontains=q)
    ...
    return render(request, 'helpApp/students_planning_to_register.html', context)

def number_of_students_registered(request):
    # Search subject
    if 'search-subject' in request.GET:
        q = request.GET.get('search-subject')
        #subject = Subject.objects.filter(subject_ID__icontains=q)
    ...
    # sent to cilent
    context = {'subject':subject, 'subject_list':subject_list}
    return render(request, 'helpApp/number_of_students_registered.html', context)
